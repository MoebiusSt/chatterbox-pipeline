...
class ConfigManager:
    ...
    def merge_speaker_params(self, base_config: Dict[str, Any], speaker_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "tts_params" in speaker_config:
            merged_config["generation"]["tts_params"] = copy.deepcopy(speaker_config["tts_params"])
        
        if "conservative_candidate" in speaker_config:
            merged_config["generation"]["conservative_candidate"] = copy.deepcopy(speaker_config["conservative_candidate"])
        
        # Set reference_audio for backward compatibility
        if "reference_audio" in speaker_config:
            merged_config["input"]["reference_audio"] = speaker_config["reference_audio"]
