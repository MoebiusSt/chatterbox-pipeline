...
def _vibevoice_resolve_attn_implementation(config) -> Optional[str]:
    """Resolve attention implementation for the nested Qwen2 language model.

    Returns None when nothing is explicitly set, so that transformers can
    auto-detect the best backend (flash_attn if available). This preserves
    the original behaviour before explicit attn overrides were introduced.
    """
    for attr in ("_attn_implementation", "attn_implementation"):
        v = getattr(config, attr, None)
        if v is not None and str(v).strip().lower() not in ("none", "null", "", "eager"):
...
class VibeVoiceModel(VibeVoicePreTrainedModel):
    def __init__(self, config):
        ...
        # When attn_impl is None, don't pass the argument so that transformers
        # auto-detects the best backend (matches the original fastest behaviour).
        lm_config = config.decoder_config
        attn_impl = _vibevoice_resolve_attn_implementation(config)
        try:
            if attn_impl is not None:
                self.language_model = AutoModel.from_config(
                    lm_config, attn_implementation=attn_impl
                )
            else:
