...
class ExecutionPlanner:
    ...
    def resolve_execution_plan(
        ...
        logger.info(
            f"Execution plan resolved: {len(plan.task_configs)} tasks, mode={plan.execution_mode}"
        )
        return plan

    def _determine_execution_context(
        self, args: Any, config_files: Optional[List[Path]]
    ) -> ExecutionContext:
        """
        ...
        if hasattr(args, "job") and args.job:
            ...
            default_config_path = (
                self.config_manager.project_root / "config" / "default_config.yaml"
            )
    ...
    def _create_execution_plan(
        ...
        if not tasks_to_process and intent.execution_mode == "single":
            ...
            if context.job_configs:
                ...
                logger.debug(
                    f"Using provided config files: {[str(p) for p in job_configs]}"
                )
            elif context.job_name:
                # Job-name execution path - find configs by name

                # Check if user might have passed a YAML file as job name
                if context.job_name.lower().endswith(
                    ".yaml"
                ) or context.job_name.lower().endswith(".yml"):
                    logger.warning(
                        f"🤔 It looks like you might have passed a YAML file as a job name: '{context.job_name}'"
                    )
                    ...
                    logger.info("")
                    logger.info(
                        "The --job option searches for jobs by name, not for YAML files."
                    )
                    logger.info(
                        "To run a specific YAML file, just pass it as an argument without --job."
                    )
                    logger.info("")

                # Find config files for this job name
                job_configs = self.config_manager.find_configs_by_job_name(
                    context.job_name
                )
                if not job_configs:
                    logger.error(
                        f"No job config found for job name: {context.job_name}"
                    )
                    return ExecutionPlan([], "cancelled")
                logger.debug(
                    f"Found config files for job '{context.job_name}': {[str(p) for p in job_configs]}"
                )
            else:
                logger.error(
                    "Cannot create new task: no job name or config files available"
                )
                return ExecutionPlan([], "cancelled")
    ...
    def _create_tasks_for_distinct_configs(
        self, job_config_paths: List[Path]
    ) -> List[TaskConfig]:
        """
        ...
        for config_path in job_config_paths:
            try:
                ...
                if not isinstance(config_path, Path):
                    logger.warning(
                        f"Skipping non-Path config: {type(config_path)} - {config_path}"
                    )
                    continue
