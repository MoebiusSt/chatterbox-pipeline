...
class TaskOptionsChoice(Enum):
    ...
    FILL_GAPS_NO_OVERWRITE = (
        "fill_gaps_no_overwrite"  # Run task, fill gaps, KEEP existing final audio
    )
    RERENDER_ALL = "rerender_all"  # Re-render ALL candidates, create new final audio
    EDIT_CANDIDATES = (
        "edit_candidates"  # Edit completed task (choose different candidates)
    )
...
class AllTasksChoice(Enum):
    ...
    ALL_FILL_GAPS = (
        "all_fill_gaps"  # Run all tasks, fill gaps, CREATE/overwrite final audio
    )
