...
def parse_args():
    ...
    # Input directories (override config)
...
def merge_args_and_config(args, config):
    ...
    merged = {
        'clean_dir': args.clean_dir or config.get('clean_dir', 'data/raw/clean'),
        'noisy_dir': args.noisy_dir or config.get('noisy_dir', 'data/raw/noisy'),
        'output_dir': args.output_dir or config.get('output_dir', 'data/processed'),
        'train_ratio': args.train_ratio or config.get('train_ratio', 0.8),
        'val_ratio': args.val_ratio or config.get('val_ratio', 0.1),
        'test_ratio': args.test_ratio or config.get('test_ratio', 0.1),
        'target_sample_rate': args.target_sample_rate or config.get('target_sample_rate', 24000),
        'file_pattern': args.file_pattern or config.get('file_pattern', '*.wav'),
        'copy_files': args.copy_files or config.get('copy_files', False),
        'validate_pairs': args.validate_pairs or config.get('validate_pairs', False),
...
def create_dataset_info(output_dir: Path, args: dict, total_pairs: int, 
